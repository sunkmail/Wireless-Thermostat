...
void updateSetpoint(void) {
  ...
  //  debugPrint("Temp. Setpoint: ");        // Display current tempSetpoint - Test confirmation
...
void doSetpoint(void) {
  ...
  if (tempData[controlNode][arrayWholeDegrees] < tempSetpoint) {      // If controlling Node temp is less than setpoint - Assumption made: inside temp will never go below zero
...
void checkDataAge(void) {
  ...
  for (byte i = 0; i <= (RFNodes - 1); i++) {               // Check each node's data
    if (tempDataRxTime[i] > (currentMillis - expiryTime)) {    // If too old ...
      debugPrint("Data invalid - Timeout, Node: ");
      debugPrint((char*)rmName[i],1);                           // Print room name for Node, char array

      tempData[i][arrayTempPos] = false;                     // Set data to invalid value, -99.9
      tempData[i][arrayWholeDegrees] = 99;
      tempData[i][arrayDecimalDegrees] = 9;
    }
  }
}
...
void isControlValid(void) {
  if(controlNode != localNode){                             // Check if using a remote node to control heating ....
    if (tempData[controlNode][arrayTempPos] = false)        // if Invalid Temp Data on Control Node  (Assumption - Inside temps will never be < zero)
...
// ------------------------------------------------------------------
void enterMenu(void) {
  debugPrint("Entering Menu System",1);
    ...
    virtualPosition = controlNode;                // Set VirtualPosition to current setting 
  ...
  debugPrint("Exiting Menu System",1);
}
...
void programSettings(void) {          
  ...
  if(virtualPosition > (RFNodes - 2)){       // Ensure Control node is valid - Outside is NOT a valid option
    virtualPosition = (RFNodes - 2);         // Therefore (node[array] - 2) allows only inside options
  }
